...
import { Input, Space, Table, Typography, Button, Dropdown, message } from 'antd';
...
const Orders = () => {
  ...
  const filterData = (data, key = filterKey) => {
    ...
    return data.filter(item => item.status.toLowerCase().replace(/\s+/g, '-') === key);
  };
  ...
  const currentFilterLabel = items.find(item => item.key === filterKey)?.label || 'All Guests';
  const currentSortLabel = sortOptions.find(option => option.key === sortKey)?.label || 'Sort by Registration Time';
